...
You must output your response using XML tags to separate the markdown report from the JSON data. Do NOT put the markdown report inside the JSON.

<markdown_report>
[Write your full Markdown report text here, formatted EXACTLY as described below. Do NOT escape newlines or quotes here.]
</markdown_report>
...
Checklist: {checklist}
"""
            raw_verified_res = self.llm.get_completion(verifier_prompt, is_json=False, enable_tools=True)
            v_data = {}
            if isinstance(raw_verified_res, str):
                md_match = re.search(r'<markdown_report>(.*?)</markdown_report>', raw_verified_res, re.DOTALL)
                json_match = re.search(r'<json_findings>(.*?)</json_findings>', raw_verified_res, re.DOTALL)
                
                if json_match:
                    try:
                        v_data = json.loads(json_match.group(1))
                    except json.JSONDecodeError as e:
                        print(f"⚠️ XML JSON parse failed: {e}")
                        clean_json = re.sub(r'^```(?:json)?\s*', '', json_match.group(1).strip())
                ...
                if md_match:
                    v_data['markdown_report'] = md_match.group(1).strip()
            
            if not v_data:
                print("⚠️ Verifier returned invalid or empty JSON/XML.")
            else:
