...
class PRReviewOrchestrator:
    ...
    def run(self) -> None:
        ...
        if not config:
            self.gh.post_failure_comment("**Missing or malformed `domain_config.json`.** Grace cannot analyze this PR without its domain rules.")
            sys.exit(1)
...
"""
        ...
        try:
            ...
            if not meta:
                self.gh.post_failure_comment("**Unable to fetch PR.** Grace could not fetch the pull request metadata.")
                sys.exit(1)
            
            pr_summary_title = meta['title']
            diff = self.gh.fetch_diff(meta['diff_url'])
            if not diff:
                self.gh.post_failure_comment("**No changes detected.** The PR diff is empty or could not be fetched by Grace.")
                sys.exit(1)
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                ...
                for future in hunter_futures:
                    ...
                    if isinstance(raw_issues, list):
                        ...
                        potential_issues.extend(raw_issues.get('findings') or 
            ...
            verifier_prompt = f"""
You are the {persona}. Your ONLY job is to deduplicate and cross-examine the findings generated by the Hunter agents, and output the final verified JSON report.
...
### 🔄 DEDUPLICATION & CROSS-EXAMINATION:
The issues provided below were generated by multiple independent Hunter agents. You MUST deduplicate identical findings and cross-examine them. Only output the unique, verified findings.

### 🛡️ STRICT SELF-CORRECTION & GROUNDING RULES (DO THIS FIRST):
{verifier_grounding_rules}

### **STRICT JSON FORMAT**:
You must output EXACTLY one valid JSON object. DO NOT output a markdown report. Use the following structure:
```json
{{
  "verified_findings": [
    {{
  ...
  "merge_verdict": "🔴 HARD STOP | 🟡 Needs Review | 🟢 LGTM"
}}
```
...
"""
            ...
            formatter_prompt = f"""You are the {persona} Markdown Formatter.
Your ONLY job is to take the structured JSON data and format it into a beautiful, strict GitHub Markdown PR comment.
...
3. **Failed Sonar Checks (If Applicable)**:
   If there are any findings that violate a SonarQube rule (based on the checklist), you MUST extract them and place them in a dedicated section titled `### [![SonarSource](https://avatars.githubusercontent.com/u/545988?s=24&v=4)](https://www.sonarsource.com/) Failed Sonar Checks`. This section MUST be placed immediately after the `Action Required` section (or right below the Top Alert Block if `Action Required` is empty).
...
Diff: {diff}
Verified JSON Data: {json.dumps(v_data, indent=2)}
Checklist: {checklist}
