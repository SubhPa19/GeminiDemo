...
class GitHubClient:
    ...
    def submit_bundled_review(self, body: str, event: str, comments: list) -> Optional[requests.Response]:
        ...
        payload = {
            "body": body + f"\n\n---\n[**Grace-{SCRIPT_VERSION}**](# \"Gemini-powered Review And Code Evaluator\") | [Share Feedback](https://forms.gle/bpRX129ku5YMi9JLA) | [Walkthrough & Guide](https://drive.google.com/file/d/1paA9hswGG1MazQF_0WBvrZD9sKBUscOy/view?usp=drive_link)\n{BOT_MARKER}",
            "event": event,
            "comments": comments
        }
...
2. **Action Required Punch List**:
   ...
   **STRICT EMPTY SECTION EXCLUSION RULE**:
    ...
    * 🟡 **Minor/Warning** | [File Name:Line Number](../blob/{meta['head_sha']}/Relative_File_Path#L<Line_Number>) — **[Issue Category Name]**: [Short description]
    Follow this action items section with extra whitespace to create a visual break (only if issues > 0). Do NOT use a markdown divider line (`---`). Use `<br>` or empty lines instead.
